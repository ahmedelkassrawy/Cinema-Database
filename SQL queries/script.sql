...
GO
/****** Object:  Trigger [dbo].[DecrementSeats]    Script Date: 5/20/2025 ******/
CREATE TRIGGER [dbo].[DecrementSeats]
ON [dbo].[Bookings]
AFTER INSERT 
AS 
BEGIN 
    ...
    JOIN inserted i ON s.Screening_id = i.screening_id
    ...
    BEGIN 
        RAISERROR ('Cannot book ticket. No seats available at this moment.', 16, 1);
...
BEGIN
    ...
    -- Update stock quantity in Concessions table
    UPDATE c
    SET c.stock_quantity = c.stock_quantity - i.Quantity
    FROM [dbo].[Concessions] c
    JOIN inserted i ON c.product_id = i.ProductID
    WHERE c.stock_quantity >= i.Quantity;

    -- Check if stock quantity became negative
    IF EXISTS (
        SELECT 1
        FROM [dbo].[Concessions] c
        JOIN inserted i ON c.product_id = i.ProductID
    ...
    BEGIN
        ...
        RAISERROR ('Cannot add concession. Insufficient stock for the product.', 16, 1);
        ROLLBACK TRANSACTION;
        RETURN;
    END;
END;
GO
/****** Object:  Trigger [dbo].[CheckSeats]    Script Date: 5/20/2025 ******/
CREATE TRIGGER [dbo].[CheckSeats]
ON [dbo].[Screenings] 
AFTER INSERT, UPDATE
AS
BEGIN 
    ...
    IF EXISTS (
        SELECT 1 
        FROM inserted i 
        JOIN [dbo].[Theaters] t ON t.Theater_id = i.Theater_id
        WHERE i.available_Seats > t.TotalSeats
    )
    BEGIN
        RAISERROR ('Available seats cannot exceed the theater total', 16, 1);
        ROLLBACK TRANSACTION;
        RETURN;
    END;
END;
